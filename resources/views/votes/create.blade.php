    ...
    @if($settings)
        @if(! $is_voted)
            <div class="container mx-auto p-4 sm:p-6 max-w-full sm:max-w-2xl lg:max-w-4xl">
                <h2 class="text-xl sm:text-2xl font-bold mb-4 text-center">نموذج التصويت</h2>
                ...
                <form method="POST" action="{{ route('votes.store') }}"
                    ...
                    @foreach($presidentCandidates as $candidate)
                        <label class="block">
                            <input type="radio" name="president_id" value="{{ $candidate->id }}" required class="mr-2">
                            {{ $candidate->name }}
                        </label>
                    @endforeach
                    ...
                    @foreach($memberCandidates as $candidate)
                        <label class="block">
                            <input type="checkbox" name="member_ids[]" value="{{ $candidate->id }}" class="mr-2">
                            {{ $candidate->name }}
                        </label>
                    @endforeach

                    <p class="text-sm text-red-600" id="member-warning" style="display: none;">يمكنك اختيار 4 أعضاء
                        فقط.</p>
                ...
                {{-- نتائج التصويت --}}
                <hr class="my-8">
                <h2 class="text-xl sm:text-2xl font-bold mb-4 text-center">نسبة التصويت لكل مرشح</h2>

                <div id="loading-msg" class="text-center text-gray-500">جاري تحميل النتائج...</div>
                <div id="live-results" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4"></div>
            </div>
                ...
                function fetchResults() {
                    fetch("{{ route('votes.results') }}")
                        ...
                        <div class="bg-white p-4 rounded shadow text-center">
                            ${candidate.photo
                                    ? `<img src="${candidate.photo}" class="w-20 h-20 sm:w-24 sm:h-24 object-cover rounded-full mx-auto mb-2">`
                                    : `<div class="w-20 h-20 sm:w-24 sm:h-24 bg-gray-300 rounded-full mx-auto mb-2 flex items-center justify-center">لا صورة</div>`}
